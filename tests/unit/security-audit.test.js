...
// Tests live in tests/ but audit sources live in the repo root.
const repoRoot = path.resolve(fileURLToPath(new URL(".", import.meta.url)), "../..");
const src = (rel) => path.resolve(repoRoot, rel);
...
describe("AUDIT-002: API key masking", () => {
  ...
  it("byApiKey response keys should not contain raw API keys", () => {
    const source = fs.readFileSync(
      src("src/lib/db/repos/usageRepo.js"),
      "utf-8"
    );
    // getUsageStats may aggregate by the full key internally (masked prefixes
    // collide — every generated key shares the sk-{machineId}- prefix), but the
    // response map must be re-keyed to a non-reversible id before returning.
    expect(source).toContain("const byApiKeySafe = {}");
    expect(source).toContain("stats.byApiKey = byApiKeySafe");
    // Row fields exposed to the client stay masked (never the raw key).
    expect(source).toContain("apiKeyMasked, keyName, apiKeyKey, apiKeyId: keyInfo?.id || null");
    expect(source).not.toMatch(/apiKey:\s*entry\.apiKey,\s*$/m);
  });
});
